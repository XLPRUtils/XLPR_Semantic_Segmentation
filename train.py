...
def parse_args():
    ...
    parser.add_argument('--device', type=str, default='1', help='The ids of GPU to be used')
...
class Trainer(object):
    def __init__(self, args):
        ...
        self.train_loader = data.DataLoader(dataset=train_dataset,batch_size=args.batch_size,shuffle=True)
        print('train dataset len: {}'.format(len(self.train_loader.dataset)))
        self.val_loader = data.DataLoader(dataset=val_dataset,batch_size=args.batch_size,shuffle=True)
        ...
        self.sch = get_lr_scheduler(type=args.lr_scheduler, opt=self.opt, max_epoch=args.epochs,iter_epoch=len(self.train_loader))
        # loss function
    ...
    def train(self):
        ...
        logger.info('Start training, Total Epochs: {:d} = Total Iterations {:d}'.format(self.args.epochs, args.max_iters))
        self.model.train()
        for epoch in range(self.args.epochs):
            ...
            for iteration, (images, targets, _) in enumerate(self.train_loader):
                ...
                if args.apex:
                    ...
                    losses.backward()
                self.opt.step()  
                eta_seconds = ((time.time() - start_time) / (iteration + iterations)) * (args.max_iters - (iteration + iterations))
                eta_string = str(datetime.timedelta(seconds=int(eta_seconds)))
                if iteration % self.args.log_iter == 0 :
                    logger.info("Iters: {:d}/{:d} || Lr: {:.6f} || Loss: {:.4f} || Cost Time: {} || Estimated Time: {}".format(
